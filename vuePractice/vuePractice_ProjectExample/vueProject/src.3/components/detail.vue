<template>
    <div class="detail">
        <div class="nav">
            <ul>
                <li @click="goback" class="l-btn"></li>
            </ul>
        </div>
<div class="content">
  ...
  <div class="cont">
    <h3>{{detail.title}}</h3>
    <div class="time"><p>{{detail.time}}<span><img src="src/assets/img/zan.png" alt=""/></span></p></div>
...
<style scoped>
  ...
  export default {
      ...
      beforeRouteEnter (to, from, next) {
        ...
        axios({
          ...
          //params:{id:this.$router.params.id}
            ...
            res => {
              next(function(_this){
                //進入路由之後，調用這個函數，傳遞組件實例，用_this接受
                _this.detail=res.data[to.params.id-1]
      ...
      //     //params:{id:this.$router.params.id}
      //   }).then(
      //     //res=>this.detail=res.data[this.$route.params.id-1]
      //       res => {
      //       setTimeout(()=>{
      //         this.detail=res.data[this.$route.params.id-1]
      //       },2000);
